...
class EnumerationController {
    constructor() {
        ...
        this.updateEnumerator = (req, res) => __awaiter(this, void 0, void 0, function* () {
            const id = req.user.id;
            const { name, phone, gender, state, lga, ward, settlement, password, healthfacility, } = req.body;
            try {
                const existingEnumerator = yield prisma.enumerator.findUnique({
                    where: { userID: id },
                });
                ...
                if (phone && phone !== existingEnumerator.phone) {
                    const phoneExists = yield prisma.enumerator.findUnique({
                        where: { phone },
                    });
                    if (phoneExists) {
                        return res.status(409).json({
                ...
                if (settlement)
                    updateData.settlement = JSON.stringify(settlement);
                if (password)
                    updateData.password = password;
                // 4. Handle health facilities (if provided)
                let healthFacilityUpdate;
                if (healthfacility) {
                    yield prisma.enumerationHealthFacility.deleteMany({
                        where: { enumeratorId: existingEnumerator.id },
                    });
                    healthFacilityUpdate = {
                        create: healthfacility.map((name) => ({ name })),
                    };
                ...
                logger_1.default.error("Error updating enumerator:", error);
                return res.status(500).json({
                    statusCode: 500,
        ...
        this.updateEnumerationData = (req, res) => __awaiter(this, void 0, void 0, function* () {
            const UserId = req.user.id;
            const _a = req.body, { clientNumber, ancVisits, tetanusVaccinationReceived } = _a, rest = __rest(_a, ["clientNumber", "ancVisits", "tetanusVaccinationReceived"]);
            try {
                const existingData = yield prisma.enumerationData.findFirst({
                    where: { clientNumber },
                });
                ...
                const updateData = Object.assign(Object.assign({}, rest), { submittedById: UserId });
                ...
                const updatedEnumeration = yield prisma.enumerationData.update({
                ...
                logger_1.default.error("Error updating enumeration data:", error);
        ...
        this.getAllEnumeratorData = (req, res) => __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const facilityNames = enumeratorInformation.healthFacility.map((hf) => hf.name);
                const whereClause = facilityNames.length > 0
                    ? {
                    ...
                    : { submittedById: userId };
                const [enumerationdata, totalCount] = yield Promise.all([
